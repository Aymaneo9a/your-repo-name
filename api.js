...
export default function App() {
  ...
  const [settings, setSettings] = useState(() => load('SETTINGS', { pdfFolder: '' }));
  ...
  const logFirstReply = useCallback((id) => {
    ...
    addIntel(`First reply logged for ${id}`, 'ti-clock', '#185FA5');
  ...
  return (
    <div className="app-shell">
      <TopBar view={view} setView={setView} onSettings={() => setShowSettings(true)} />
      <SLABar counts={counts} onFilter={setTicketFilter} activeFilter={ticketFilter} onNewTicket={() => setShowNewTicket(true)} />

      {view === 'dashboard' && (
        <div className="main-grid">
          <WarRoom tickets={filteredTickets} activeId={activeTicketId} onSelect={setActiveTicketId} />
          <Workspace
            ticket={activeTicket}
          ...
          />
          <RightPanel kpi={kpi} intel={intel} />
        </div>
      )}

      {view === 'kpis'      && <KPIView kpi={kpi} tickets={tickets} />}
      {view === 'knowledge' && <KnowledgeView kb={kb} setKb={setKb} onIntel={addIntel} />}
